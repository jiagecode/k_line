  ...
  <div id="eerchant" class="app-container">
    <div class="app-box">
      <div class="app-box-tab">
        <div class="app-box-title">商户管理</div>
        <div class="app-box-changeBox">
          ...
          <div class="app-box-input app-marginR">
            <div class="app-box-input-txt">商户ID：</div>
            <el-input placeholder="请输入商户ID" v-model="input1"></el-input>
          </div>
          <div class="app-box-input app-marginR">
            <div class="app-box-input-txt">手机号码：</div>
            <el-input placeholder="请输入手机号码" v-model="input2"></el-input>
          </div>
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <!--复制-->
                <span v-if="item.prop==='sysUserId'" @click="handleCopy(scope.row.sysUserId)">
                             {{scope.row.sysUserId}}
                </span>
                <span v-else-if="item.prop==='phone'" @click="handleCopy(scope.row.phone)">
                             {{scope.row.phone}}
                            </span>
                <span v-else-if="item.prop==='merchantInfo.merchantName'">
                ...
                <span v-else-if="item.prop==='merchantInfo.merchantAmount'">
                             {{scope.row.merchantInfo['merchantAmount']}}
                            </span>
                <span v-else-if="item.prop==='merchantInfo.merchantWithholdAmount'">
                             {{scope.row.merchantInfo['merchantWithholdAmount']}}
                            </span>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false"
      ...
      <div class="payNameDiaBox">
        <el-form ref="form" :model="form" label-width="130px" :rules="rules">
          <el-form-item label="商户名称：" prop="username" v-if="!bjShow">
    ...
    data() {
      ...
      return {
        ...
        tabHead: [
          ...
          {
            label: '商户名称',
            prop: 'merchantInfo.merchantName'
          },
          {
            label: '平台名称',
            prop: 'merchantInfo.platformName'
          },
          {
            label: '商户余额',
            prop: 'merchantInfo.merchantAmount'
          },
          {
            label: '代付预扣款',
            prop: 'merchantInfo.merchantWithholdAmount'
          },
          {
            label: '手机号码',
            prop: 'phone'
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            var data = {
              merchantInfo: {
                merchantName: this.form.username,
                platformName: this.form.ptname,
                merchantType: this.telRoleid,
                merchantInfoId: this.form.merchantInfoId
              },
              phone: this.form.tel,
              remarks: this.form.txt,
              officeId: 1,
              roleId: this.telRoleid,
